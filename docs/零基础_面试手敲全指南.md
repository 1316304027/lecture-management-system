...
## 附录 D：2026-06-24 功能改进（実績レポート・聊天未読・コース管理）

### D.1 実績レポート（成绩逻辑修正）

**以前的问题**：学生提交了作业但讲师还没打分，平均スコア显示 **0.0点**（红色）——不合理。
...
|------|------|
| 採点済 | 讲师已打分的提交数 |
| 採点待ち | 已提交但 score 还是 null 的数量 |
| 平均スコア | **只算已打分的平均**；若一件都没打分 → 显示「採点待ち」徽章，不是 0 点 |

**代码位置**：`ReportService.calcGradedAverage()` — 只 filter `score != null`

**面试怎么说**：
> 未採点の提出は平均に含めず、採点待ちとして別表示しました。

### D.2 チャット未読红点（不用 Lambda）

**表**：`chat_read_status`（user_id + course_id + last_read_at）

**流程**：
```
别人发消息 → chat_messages 表 INSERT
你打开聊天页 → ChatService.markCourseAsRead() 更新 last_read_at
ホーム加载 → ChatService.buildUnreadMap() 统计 sentAt > lastReadAt 且 sender ≠ 自己
チャット按钮上显示红色数字
```

**代码文件**：
- `ChatReadStatus.java` / `ChatReadStatusRepository.java`
- `ChatService.countUnread()` / `buildUnreadMap()`
- `ChatController` 打开页面时 markAsRead
- `student-home.html` / `instructor-home.html` 红色 `.pcfa-chat-badge`

**为什么不用 Lambda**：研修期内用 DB 既読フラグ更简单可靠，Lambda 是发表后的扩展。

### D.3 コース管理 UI 大改

**问题**：画面乱、浏览器自带 `input type="time"` 时和分对不齐。

**解决**：
- 用 `fragments/time-picker.html`：**小时下拉 + 分钟下拉**（08〜21 时，00/15/30/45 分）
- `AdminController.parseOptionalTime(startHour, startMinute)` 合并成 LocalTime
- 页面分区：`pcfa-admin-section`（基本情報 / 講師 / 学生 / 授業日）
- 去掉杂乱的多色 card-header，统一绿色顶栏区块
...
*文档版本：2026-06-24 | 含実績レポート・未読チャット・コース管理改进*
